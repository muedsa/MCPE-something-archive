...
class Unbreak implements Plugin {
	...
	public function __construct(ServerAPI $api, $server = false){
   	...
   	public function init(){
   		...
   		$this->api->console->register("unb","禁止破坏方块", array($this, "CMDevent"));
   		$this->api->console->register("reb","解锁方块禁止破坏", array($this, "CMDevent"));
   	...
   	public function CMDevent($cmd, $args, $issuer){
   		...
   		switch($cmd){
   			case "unb" :
            if(isset($this->lock[$issuer->iusername])){
               unset($this->lock[$issuer->iusername]);
               $output .= "[Unbreak] 模式解除";
            }else{
