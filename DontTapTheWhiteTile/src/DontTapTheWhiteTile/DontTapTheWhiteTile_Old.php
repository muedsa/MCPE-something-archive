...
class DontTapTheWhiteTile extends PluginBase implements Listener{
	...
	public function onLoad(){
		$this->getLogger()->info(TextFormat::GREEN . "小游戏 *别踩白块儿* 正在加载 !");
	}
	...
	public function onEnable(){
		...
		$this->getLogger()->info(TextFormat::DARK_GREEN . "小游戏 *别踩白块儿* 开启 !");
    }

	public function onDisable(){
		$this->getLogger()->info(TextFormat::DARK_RED . "小游戏 *别踩白块儿* 关闭 !");
	}
				...
				switch ($args[0]) {
					case 'set':
						if($this->config->getAll() !==  array()){
							$sender->sendMessage("小游戏 *别踩白块儿* 已经设置,请使用命令 : /dttt del 来删除设置!");
						}else{
							$name = $sender->getName();
							$this->SetStatus[$name] = 0;
							$sender->sendMessage("你已经处于设置状态下,请先设置一个4(宽)*5(高)的方框\n然后点击四个牌子作为开始按钮和排行榜");
						}
						break;
					...
					case 'del':
						...
						unset($this->Top2);
						unset($this->Top3);
						$sender->sendMessage("清除了小游戏 *别踩白块儿* 的设置");
						break;
