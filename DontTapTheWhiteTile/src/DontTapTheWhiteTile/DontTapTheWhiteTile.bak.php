...
class DontTapTheWhiteTile extends PluginBase implements Listener{
	...
	public function onLoad(){
		...
		if(!file_exists($this->getDataFolder()."resources/game.MUsql")){
			...
			$this->gamedb = new \SQLite3($this->getDataFolder()."resources/game.MUsql", SQLITE3_OPEN_READWRITE | SQLITE3_OPEN_CREATE);
			$this->gamedb->exec("CREATE TABLE gametable ( name TEXT PRIMARY KEY, face TEXT, x INTEGER, y INTEGER, z INTEGER, level TEXT );");
			$this->gamedb->exec("CREATE TABLE startsign ( name TEXT PRIMARY KEY, x INTEGER, y INTEGER, z INTEGER, level TEXT , face INTEGER );");
		}else{
			$this->gamedb = new \SQLite3($this->getDataFolder()."resources/game.MUsql", SQLITE3_OPEN_READWRITE);
		}
		if(!file_exists($this->getDataFolder()."resources/top.MUsql")){
			$this->getLogger()->info(TextFormat::DARK_GREEN . "Building [Top] Database!");
			$this->topdb = new \SQLite3($this->getDataFolder()."resources/top.MUsql", SQLITE3_OPEN_READWRITE | SQLITE3_OPEN_CREATE);
			$this->topdb->exec("CREATE TABLE top ( name TEXT PRIMARY KEY, time REAL );");
		}else{
			$this->topdb = new \SQLite3($this->getDataFolder()."resources/top.MUsql", SQLITE3_OPEN_READWRITE);
		}
	}

	public function loadGameConfig(){
		$this->getLogger()->info(TextFormat::YELLOW . "Loading [MiniGame] Config!");
		$results_gametable = $this->gamedb->query('SELECT * FROM gametable');
		while($row_gametable = $results_gametable->fetchArray(SQLITE3_ASSOC)) {
			if ($row_gametable["face"] == "z") {
				if($row_gametable["face"] == "x") {
					for ($i=0; $i < 5; $i++) { 
						for ($n=0; $n < 4; $n++) { 
							$this->gametable[$row_gametable["name"]][$i][$n] = array(
																		"x" => $row_gametable["x"],
																		"y" => $row_gametable["y"] + $i,
																		"z" => $row_gametable["z"] + $n,
																		"level" => $row_gametable["level"],
								);
						}
					}
				}
					...
					switch ($args[0]) {
						case 'set':
							if ($sender instanceof Player) {
								if ($sender->isOp()) {
									if(isset($args[1])){
										$prepare = $this->gamedb->prepare("SELECT * FROM gametable WHERE name = :name");
						...
						case 'del':
							if ($sender->isOp()) {
								if(isset($args[1])){
									...
									$results = $prepare->execute();
									if($results instanceof \SQLite3Result){
										$prepare1 = $this->gamedb->prepare("DELETE FROM gametable WHERE name = :name");
						...
						case 'list':
							...
							if ($sender->isOp()) {
								$prepare = $this->gamedb->prepare("SELECT * FROM gametable");
								$results = $prepare->execute();
								if($results instanceof \SQLite3Result){
